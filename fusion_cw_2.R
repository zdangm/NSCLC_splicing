...
suppressMessages(library('data.table'))#new add
...
option_list = list(
  ...
  make_option("--extract", action="store", default=NA, type='character',
              help="extract snp"),
  make_option("--blsmm_output", action="store", default=NA, type='character',
              help="extract blsmm weights"),
...
#BSLMM
 weights.bslmm = function( input , bv_type , snp , out=NA ) {
  ...
  arg = paste(opt$PATH_gemma," -miss 1 -maf 0 -r2 1 -rpace 1000 -wpace 1000 -bfile " , input , " -bslmm " , bv_type , " -o " , opt$region , sep='' )
  system( arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
  #system("ln -s /share/data/lungca_RNAseq/splicing/06_spTWAS/02_Fusion/03_output/tmp/ /share/data/lungca_RNAseq/splicing/06_spTWAS/src/output/")
  eff = read.table( paste(opt$blsmm_output,".param.txt",sep=''),head=T,as.is=T)
...
hsq.file = opt$hsq_set
hsq_file = as.data.frame(fread(hsq.file))
unique_intron <- unique(hsq_file$intron)
hsq.pv = as.numeric(hsq_file[1,2])
hsq = as.numeric(hsq_file[1,3])

geno.file = opt$tmp

# --- load pheno
pheno.file = opt$pheno
pheno = as.data.frame(fread(pheno.file))
# Match up data
...
# if(identical(colnames(pheno)[3:length(pheno)], hsq_file$intron)) {
#   cat("### intron matrix for heri and pheno are the same\n")
# }

pheno = pheno[,c(1,2,3)]

if(identical(colnames(pheno)[3],hsq_file$intron[1])) {
  cat(paste0("### intron——",colnames(pheno)[3]," under running for heri and pheno are the same\n"))
}


############## ---recode to the intersection of samples and new phenotype
arg = paste( opt$PATH_plink ," --allow-no-sex --bfile ",opt$bfile," --pheno ",pheno.file," --extract ",opt$extract," --make-bed --out ",geno.file,sep='')
system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
...
for ( mod in 1:M ) {
  if ( models[mod] == "blup" ) {
    wgt.matrix[,mod] = weights.bslmm( geno.file , bv_type=2 , snp=genos$bim[,2] , out=opt$region )
  }
  else if ( models[mod] == "bslmm" ) {
    wgt.matrix[,mod] = weights.bslmm( geno.file , bv_type=1 , snp=genos$bim[,2] , out=opt$region )
  }		
  else if ( models[mod] == "lasso" ) {
